    ...
    left_door(leftDoorServo), right_door(rightDoorServo)
{
    ...
    left_door_servo.write(0); //start with doors closed
    right_door_servo.write(180); //start with doors closed
    delay(200);
}
void SideDoors::leftDoorWrite(int angle){
    left_door_servo.write(angle);
}
void SideDoors::rightDoorWrite(int angle){
    right_door_servo.write(180-angle);
}
